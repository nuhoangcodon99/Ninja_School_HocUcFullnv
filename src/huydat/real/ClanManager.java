...
public class ClanManager {
    ...
    public static void close() {
        ...
        System.err.println("Flush/ Close ClanManager");
    }
    
    public void loadClan(){
        try {
            SQLManager.stat.executeUpdate("UPDATE `clan` SET `EventClan`=1 WHERE `id`=" + this.id + " ;");
        } catch (Exception e) {
        }
    }
    ...
    public void truluot(){
        try {
            SQLManager.stat.executeUpdate("UPDATE `clan` SET `EventClan`=0 WHERE `id`=" + this.id + " ;");
        } catch (Exception e) {
        }
    }
}
