...
public class GameCanvas {
    ...
    public static void getDataEffect(Session session, short id) {
        ...
        try {
            if(session != null) {
                ...
                ResultSet rs = SQLManager.stat.executeQuery("SELECT * FROM `vhd_data`.`Effdata` WHERE `id` = " + id + ";");
                byte[] ab;
                rs.last();
                if (rs.getRow() == 0) {
    ...
    public static byte[] Getdata(ResultSet rs, short id) {
        ...
        try {
            rs = SQLManager.stat.executeQuery("SELECT * FROM `vhd_data`.`Effdata` WHERE `id` = " + id + ";");
            while (rs.next()) {
