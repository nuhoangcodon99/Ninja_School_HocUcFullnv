            ...
            if(player != null && player.c != null && m != null && m.reader().available() > 0) {
                ...
                if(chat.equals("info")) {
                    ...
                    Server.manager.sendTB(player, "Sự kiện Noel", noel);
                     ...
                     player.conn.sendMessageLog("MapID: " + player.c.mapid + " - X: " + player.c.get().x + " - Y: " + player.c.get().y);
                player.conn.sendMessageLog(chat);
            }
                 ...
                 else if (chat.equals("i")) {
                     player.conn.sendMessageLog("MapID: " + player.c.mapid + " - X: " + player.c.get().x + " - Y: " + player.c.get().y);
               
            }
                  else if (chat.equals("update")) {
                      Server.manager.preload();
                      player.conn.sendMessageLog("thành công.");
    ...
    public static void wakeUpDieReturn(Player p) {
        if(p != null && p.conn != null && p.c != null && p.c.isDie && p.c.tileMap != null) {
            ...
            if (!tileMap.map.LangCo() && tileMap.map.dun == null) {
                if (p.luong < 5) {
                    p.conn.sendMessageLog("Bạn không đủ 5 lượng để hồi sinh!");
                } else {
                    p.c.get().isDie = false;
                    p.luongMessage(-5L);
            ...
            if(p != null && p.c != null && p.conn != null && m != null && m.reader().available() > 0) {
                ...
                Item item = p.c.getIndexBag(index);
                  if (item.quantity > 32000) {
                        p.conn.sendMessageLog("Bán con cặc gì nhiều thế :)))");
                        return;
                    }
                    ...
                    for(i = 0; i < player.sortNinja.length; ++i) {
                        if (name.equals(player.sortNinja[i])) {
                            ...
                            if (player.c != null) {
                                ...
                                player.sendInfo();
                                if (player.c.taskIndex != -1) {
                                player.c.sendmaintask();
                                }
