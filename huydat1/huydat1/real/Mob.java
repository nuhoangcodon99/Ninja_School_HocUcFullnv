...
public class Mob {
    ...
    private static int[] arrMobLangCoId = new int[] {148, 146, 147, 148, 149, 151, 152, 154, 155, 156, 157, 159,181,182,183 };
  ...
  public boolean checkMobhuydat() {
    ...
    public void handleAfterCharFight(Char _char) {
        ...
        if(this.templates.id == 230 && this.tileMap.map.bossTuanLoc != null) {
          ...
          if (_char.get().ItemBody[_char.c.taskDanhVong[3]] != null && _char.isTaskDanhVong == 1 && _char.taskDanhVong[0] == 7 && Math.abs(this.level - _char.get().level) <= 10) {
                _char.taskDanhVong[1]++;
                if(_char.c.taskDanhVong[1] < _char.c.taskDanhVong[2]) {
                        _char.p.sendAddchatYellow("- Sử dụng trang bị "+ItemTemplate.ItemTemplateId(_char.c.ItemBody[_char.c.taskDanhVong[3]].id).name+". "+"- Tiêu diệt "+_char.c.taskDanhVong[1]+"/"+_char.c.taskDanhVong[2]+ "quái lệch 10 cấp độ.");
                }
                if(_char.c.taskDanhVong[1] == _char.c.taskDanhVong[2]) {
                    _char.p.sendAddchatYellow("Hoàn thành nhiệm vụ, hãy gặp Ameji để trả nhiệm vụ");
                }
            }
            int master = this.sortNinjaFight();
             if (this.lvboss == 1) {
                this.tileMap.numTA--;
                if (_char.get().ItemBody[_char.c.taskDanhVong[3]] != null && _char.isTaskDanhVong == 1 && _char.taskDanhVong[0] == 8 && Math.abs(this.level - _char.get().level) <= 10) {
                    _char.taskDanhVong[1]++;
                    if(_char.c.taskDanhVong[1] < _char.c.taskDanhVong[2]) {
                        _char.p.sendAddchatYellow("- Sử dụng trang bị "+ItemTemplate.ItemTemplateId(_char.c.ItemBody[_char.c.taskDanhVong[3]].id).name+". "+"- Tiêu diệt "+_char.c.taskDanhVong[1]+"/"+_char.c.taskDanhVong[2]+ "quái tinh anh lệch 10 cấp độ.");
                    }
                    if(_char.c.taskDanhVong[1] == _char.c.taskDanhVong[2]) {
                    _char.p.sendAddchatYellow("Hoàn thành nhiệm vụ, hãy gặp Ameji để trả nhiệm vụ");
                    }
                ...
                this.tileMap.numTL--;
                if (_char.get().ItemBody[_char.c.taskDanhVong[3]] != null && _char.isTaskDanhVong == 1 && _char.taskDanhVong[0] == 9 && Math.abs(this.level - _char.get().level) <= 10) {
                    _char.taskDanhVong[1]++;
                    if(_char.c.taskDanhVong[1] < _char.c.taskDanhVong[2]) {
                        _char.p.sendAddchatYellow("- Sử dụng trang bị "+ItemTemplate.ItemTemplateId(_char.c.ItemBody[_char.c.taskDanhVong[3]].id).name+". "+"- Tiêu diệt "+_char.c.taskDanhVong[1]+"/"+_char.c.taskDanhVong[2]+ "quái thủ lĩnh lệch 10 cấp độ.");
                    }
                    if(_char.c.taskDanhVong[1] == _char.c.taskDanhVong[2]) {
                    _char.p.sendAddchatYellow("Hoàn thành nhiệm vụ, hãy gặp Ameji để trả nhiệm vụ");
                    }
  ...
  if (Math.abs(this.level - _char.get().level) <= 10 || this.tileMap.map.mapvhd1() || this.tileMap.map.mapChienTruong()) {
                if (this.lvboss == 1) {
           ...
           else if (this.tileMap.map.mapvhd1()) {
                ItemLeave.randomLeave(this.tileMap, this, master, Util.nextInt(1, 3), 1);
                _char.p.upluongMessage(1);
                if(this.lvboss == 2) {
                    ItemLeave.leaveTTTT(this.tileMap, this, master);                  
