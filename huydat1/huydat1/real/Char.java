        ...
        try {
            synchronized(Server.LOCK_MYSQL) {
                ...
                if (red != null && red.first()) {
                    ...
                    nja.taskId = red.getByte("taskId");
                    nja.taskIndex = red.getInt("taskIndex");
                    ...
                    nja.pointBossChuot = red.getInt("pointBossChuot");
                      nja.pPhatQua = red.getInt("pPhatQua");
    ...
    public void flush() {
        ...
        try {
            synchronized(Server.LOCK_MYSQL) {
                ...
                String sqlSET = "`head`=" + this.head +",`caiTrang`=" + this.caiTrang +",`rankTDB`=" + this.rankTDB +",`countWin`=" + this.countWin +",`countPhao`=" + this.countPhao +",`nhanTP`=" + this.nhanTP +",`countTDB`=" + this.countTDB + ",`isGiftTDB`=" + this.isGiftTDB + ", `taskId`=" + this.taskId + ", `taskIndex`=" + this.taskIndex + ",`class`=" + this.get().nclass + ",`ppoint`=" + this.get().ppoint + ",`potential0`=" + this.get().potential0 + ",`potential1`=" + this.get().potential1 + ",`potential2`=" + this.get().potential2 + ",`potential3`=" + this.get().potential3 + ",`spoint`=" + this.get().spoint + ",`level`=" + this.get().level + ",`exp`=" + this.exp + ",`expdown`=" + this.expdown + ",`expSkillClone`=" + this.expSkillClone + ",`pk`=" + this.pk + ",`xu`=" + this.xu + ",`yen`=" + this.yen + ",`maxluggage`=" + this.maxluggage + ",`levelBag`=" + this.levelBag + ",`site`='" + jarr.toJSONString() + "', `buyX3` =" + this.countBuyX3;
                jarr.clear();
            ...
            switch (taskId) {
                case 0: {
                    if (taskIndex != -1) {
                        ...
                        msg.writer().writeUTF("- Nhiệm Vụ Chào Làng:");
                        msg.writer().writeUTF("\nCó làm thì mới có ăn nhớ chưa con!!");
                        msg.writer().writeByte(7);
                        msg.writer().writeUTF(" Nói chuyện với Kiriko");
                        msg.writer().writeUTF(" Nói chuyện với Tabemono");
                        msg.writer().writeUTF(" Nói chuyện với Kenshinto");
                        msg.writer().writeUTF(" Nói chuyện với Okanechan");
                        msg.writer().writeUTF(" Nói chuyện với Kamakura");
                        msg.writer().writeUTF(" Nói chuyện với Umayaki");
                        msg.writer().writeUTF(" Gặp Tajima báo cáo");
                        msg.writer().writeShort(0);
                        short[] a = new short[]{-1, -1, -1, -1, -1, -1, -1};
                        for (int i = 0; i < taskIndex; i++) {
                            a[i] = 1;
                        }
                        for (int i = 0; i < taskIndex; i++) {
                            msg.writer().writeShort(a[i]);
                        }
                        this.p.conn.sendMessage(msg);
                    } else if (taskIndex == -1) {
                        msg.writer().writeShort(taskId);
                        msg.writer().writeByte(taskIndex);
                        msg.writer().writeUTF("Hãy tới nói chuyện với già làng");
                        msg.writer().writeUTF("\nChưa biết ở đâu thì vào box zalo hỏi");
                ...
                case 8: {
                    if (taskIndex != -1) {
                        msg.writer().writeShort(taskId);
                        msg.writer().writeByte(taskIndex);
                        msg.writer().writeUTF("- NV Tìm Hiểu 3 Trường:");
                        msg.writer().writeUTF("Tìm đến trường và nói chuyện với thầy cô\nCó làm thì mới có ăn nhớ chưa con!!");
                        msg.writer().writeByte(5);
                        msg.writer().writeUTF("Trình độ yêu cầu: k bị ngu là đc");
                        msg.writer().writeUTF(" Tìm hiểu trường Hirosaki");
                        msg.writer().writeUTF(" Tìm hiểu trường Ookaza");
                        msg.writer().writeUTF(" Tìm hiểu trường Haruna");
                        msg.writer().writeUTF(" Gặp Tajima báo cáo");
                        msg.writer().writeShort(0);
                        short[] a = new short[]{ -1, -1, -1, -1, -1 };
                        for (int i = 0; i < taskIndex; i++) {
                            a[i] = 1;
                        }
                        for (int i = 0; i < taskIndex; i++) {
...
//                        this.p.conn.sendMessage(msg);
    ...
    public void finishnv() {
        Message msg = new Message(49);
        this.p.conn.sendMessage(msg);
        msg.cleanup();
    }
