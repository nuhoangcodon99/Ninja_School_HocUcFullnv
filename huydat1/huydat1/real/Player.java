...
public class Player extends User{
    ...
    if (!Util.CheckString(this.tenmoi + this.tencu, "^[a-zA-Z0-9]+$") || this.tenmoi.length() < 3 || this.tenmoi.length() > 15) {
        this.conn.sendMessageLog("Tên chỉ đồng ý các ký tự a-z,0-9 và chiều dài từ 3 đến 15 ký tự");
        return;
    }

    try {
                         
        final ResultSet red = SQLManager.stat.executeQuery("SELECT `id` FROM `ninja` WHERE (`name` LIKE '" + this.tencu + "' AND `id` = " + this.id + ");");
        if (red == null || !red.first()) {
            this.conn.sendMessageLog("Tên cũ không chính xác!");
            return;
        }
        synchronized (Server.LOCK_MYSQL) {
            SQLManager.stat.executeUpdate("UPDATE `player` SET `ninja`='" + this.tenmoi + "' WHERE `id`=" + this.id + " LIMIT 1;");
            SQLManager.stat.executeUpdate("UPDATE `ninja` SET `name`='" + this.tenmoi + "' WHERE `id`=" + this.id + " LIMIT 1;");
        }
        this.conn.sendMessageLog("Đã đổi tên thành công thành ");
    } catch (SQLException e) {
        e.printStackTrace();
    }
}
...
public static void sendEffectAuto(Player p, byte id, int x, int y, byte count, short timeInSec) {
        Message m = null; 
         try {
        m = new Message(122);
        m.writer().writeShort(1);
    ...
    public boolean updateXpMounts(int xpup, byte type) {
        ...
        if (item == null) {
            ...
            return false;
        } else if (type == 3 && item.id != 778 && item.id != 828 && item.id != 968) {
    ...
    public void sendInfo2() {
        ...
        try {
            ...
            m.writer().writeByte(this.c.gender);
            m.writer().writeShort(this.c.partHead());
            m.writer().writeByte((int) this.c.speed());
            m.writer().writeUTF(this.c.name);
            m.writer().writeByte(this.c.pk);
            m.writer().writeByte(this.c.typepk);
            m.writer().writeInt((int) this.c.getMaxHP());
            m.writer().writeInt((int) this.c.hp);
            m.writer().writeInt((int) this.c.getMaxMP());
            m.writer().writeInt((int) this.c.mp);
            m.writer().writeLong(this.c.exp);
            m.writer().writeLong(this.c.expdown);
            m.writer().writeShort((int) this.c.eff5buffHP());
            m.writer().writeShort((int) this.c.eff5buffMP());
            m.writer().writeByte(this.c.nclass);
            m.writer().writeShort(this.c.ppoint);
            m.writer().writeShort((int) this.c.potential0);
            m.writer().writeShort((int) this.c.potential1);
            m.writer().writeInt((int) this.c.potential2);
            m.writer().writeInt((int) this.c.potential3);
            m.writer().writeShort(this.c.spoint);
            m.writer().writeByte(this.c.skill.size());
