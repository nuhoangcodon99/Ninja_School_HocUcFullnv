...
public class Rank {
   ...
   public static ArrayList<Entry4> bxhdiemhoa = new ArrayList<>();
    ...
    public static void init() {
        ...
        System.err.println("Cập Nhập Bảng Xếp Hạng!");
        for (int i = 0; i < Rank.bangXH.length; ++i) {
            initBXH(i);
        }
    }
        ...
        switch (type) {
            ...
            case 5: {
               ...
               case 11: {
                ...
                try {
                    int i = 1;
                    red = SQLManager.stat.executeQuery("SELECT `name`,`diemct` FROM `ninja` WHERE (`diemct` > 0) ORDER BY `diemct` DESC LIMIT 10;");
                    String name;
                    int point;
                    Entry bXHHL;
                    while (red.next()) {
                        name = red.getString("name");
                       int sk = red.getInt("diemct");
                        bXHHL = new Entry();
                        bXHHL.nXH = new long[1];
                        bXHHL.name = name;
                        bXHHL.index = i;
            ...
            case 5:{
              ...
              if (bangXH[type].isEmpty()) {
                ...
                if (bangXH[type].isEmpty()) {
                    ...
                    for (Entry bxh : bangXH[type]) {
                        str += bxh.index + ". " + bxh.name + " có " + Util.getFormatNumber(bxh.nXH[0]) + " điểm Chiến Thần .\n";
                    }                 
                }
                break;
            }
